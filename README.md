...
## ゲーム性を高めるための提案
- **テンポの波を演出する短期イベント**: 60秒ごとに「ブリッツタイム」などの短時間イベントを自動抽選し、出現速度・クリティカル率・コンボ猶予のいずれかをランダム強化。演出用の色替えやSEを追加すると、何度でもプレイしたくなるリズムが生まれます。
- **目的地を示すデイリー目標**: 「3分以内にコンボ20回」「FEVERを5回発動」「ルミナスバーストを3回撃つ」などの日替わりチャレンジを提示し、達成でジェム倍率や一時的な自動収入ブーストを付与。短期の明確なゴールがあると継続率が向上します。
...
- **カムバック報酬の余韻**: 復帰時に「オフライン収益」を計算してまとめて獲得させ、同時にFEVERゲージを一定割合チャージ。戻ってすぐに派手な状態を体験できると、再ログインの動機付けになります。
...
| --- | --- | --- |
| 短期イベント（ブリッツタイム） | 中 | ゲームループへ乱数イベントを挿入する必要があり、発生タイミングと同時発動の競合管理がやや複雑。パラメータ調整のテストコストが高め。 |
| デイリー目標 | 中 | 目標の保存／日付リセットをブラウザストレージで扱う必要があり、時刻依存バグに注意。UI一覧と報酬配布の導線を整理すれば拡張しやすい。 |
